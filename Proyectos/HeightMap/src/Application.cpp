...
void Application::setupProgram2()
{
	...
	ids["heightMap"] = glGetUniformLocation(ids["program2"], "heightMap"); //Height Map
...
void Application::setup()
{
	...
	ids["Diffuse"] = setupTexture("Textures/Diffuse Map.png");
	ids["Height"] = setupTexture("Textures/Height Map.png");
...
void Application::draw()
{
	...
	modelRot = glm::rotate(modelRot, rotX, glm::vec3(1.0f, 0.0f, 0.0f)); //rotX
	modelRot = glm::rotate(modelRot, rotY, glm::vec3(0.0f, 1.0f, 0.0f)); //rotY

	glUniform1f(ids["time2"], time);

	glUniformMatrix4fv(ids["model"], 1, GL_FALSE, &modelRot[0][0]);
	...
	glBindTexture(GL_TEXTURE_2D, ids["Height"]);
